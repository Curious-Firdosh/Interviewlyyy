...
const SessionPage = () => {
  ...
  const problemData = session?.problem
    ...
    // auto-join session if user is not already a participant and not the host
     useEffect(() => {
        ...
        if (hasAutoJoinedRef.current) return; // prevent multiple auto-joins
        console.log("that is run successfullly");
  ...
  useEffect(() => {
        ...
        if (session.status === "Compleated") navigate("/dashboard");
  ...
  const handleRunCode = async () => {
    ...
    const result = await execCode(selectedLanguage, code);
    setOutput(result);
    setIsRunning(false);
  };
